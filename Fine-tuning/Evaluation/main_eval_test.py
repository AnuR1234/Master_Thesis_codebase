...
class ABAPEvaluationFramework:
    ...
    def run_phase_5_llm_evaluation(self, results: Dict):
        ...
        # Check if this is test mode (small dataset)
        total_samples = sum(len(model_results) for model_results in results.values())
        is_test_mode = total_samples <= 10  # Consider it test mode if 10 or fewer samples
        
        for model_name, model_results in results.items():
            # Limit samples for LLM evaluation (can be expensive)
            if is_test_mode:
                limited_results = model_results[:3]  # Only 3 samples for quick test
                self.logger.info(f"🧪 Test mode: LLM evaluating {model_name} ({len(limited_results)} samples only)")
            else:
                limited_results = model_results[:20]  # Evaluate first 20 samples for full run
