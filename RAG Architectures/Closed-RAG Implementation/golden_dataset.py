...
def save_dataset(dataset, base_filename):
    """Save dataset in both JSON and CSV formats using the current directory"""
    if not dataset:
    ...
    print(f"✅ Saved JSON file: {os.path.join(current_dir, json_path)}")
    ...
    print(f"✅ Saved CSV file: {os.path.join(current_dir, csv_path)}")
    ...
    print(f"\n{json_path}: {json_size:.2f} KB")
    print(f"{csv_path}: {csv_size:.2f} KB")
    print("====================\n")
...
if __name__ == "__main__":
    try:
        ...
        asyncio.run(generate_all_raget_datasets(questions_per_type=15))  # Default set to 40 questions per type
